...
class TCClient():
    ...
    def __init__(self, host, port, classifier=""):
        self.host = host
        self.port = port
...
if __name__=="__main__":
    ...
    #content = ""
    client = TCClient("192.168.23.32", 9090)
    client.connect()
    print client.checkRecType(title, content)
    processResult = client.process("上个月单日上涨幅度大于9%", "上个月单日上涨幅度大于9%",additionalInformation="classifiersName=question")
    print processResult
